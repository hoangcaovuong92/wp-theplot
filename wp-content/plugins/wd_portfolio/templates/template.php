...
function show_wd_portfolio( $columns = 4,$show_filter = "yes"/*, $style="padding"*/, $show_title = "yes" ,$show_desc = "yes", $count = "-1", $portf_style = 'style1',$show_pages="yes" ){
	$wd_portfolio = new WD_Portfolio();
	//$category = '';$title = ''; $number_posts = "8" ;$style2 = "no-padding";
?>
					<?php if($portf_style =="style3")
					{
						 echo show_wd_portfolio_mansory($columns,$show_filter,$show_title,$show_desc,$count,$portf_style,$show_pages);
					}
					else { ?>
						<div id="portfolio-container" class="<?php echo "portfolio-".$portf_style."-style" ?>">
							...
							<div class="end_content">
							  ...
							  <div class="page_navi"><?php tvlgiao_wpdance_ew_pagination(); wp_reset_query();?></div>
							  <?php }?>
							</div>
						</div>				
...
function show_item_portfolio($id = '', $slug = ''){
	...
	if($query->have_posts()) : 
		while($query->have_posts()) : $query->the_post();
			...
			$post_url =  esc_url(get_permalink($post->ID));
			$url_video = esc_url(get_post_meta($post->ID,TVLGiao_Wpdance_THEME_SLUG.'video_portfolio',true));
			$thumb=get_post_thumbnail_id($post->ID);
