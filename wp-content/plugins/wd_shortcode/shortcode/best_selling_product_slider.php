...
if(!function_exists('wd_best_selling_product_slider_function')){
	function wd_best_selling_product_slider_function($atts,$content){
		...
		tvlgiao_wpdance_shop_loop_prod_remove_action($data);
		...
		if ( $products->have_posts() ) : ?>
						...
						<div class="product-slider-body <?php if($box_style == 'style-boxed') echo "col-sm-24";?>">
							...
							<div class="wd-big-product product<?php echo $extra_class?>">
								...
								<div class="big-product-button-list">
									<?php tvlgiao_wpdance_wd_list_template_loop_add_to_cart();?>
									<?php 
										if( class_exists('YITH_WCWL_UI') && class_exists('YITH_WCWL') && function_exists('tvlgiao_wpdance_wd_add_wishlist_button_to_product_list_shortocode') ){
											tvlgiao_wpdance_wd_add_wishlist_button_to_product_list_shortocode();
										}
										if( class_exists( 'YITH_Woocompare_Frontend' ) && class_exists( 'YITH_Woocompare' ) && function_exists('tvlgiao_wpdance_wd_add_compare_link') ){
							...
							<div class="product-slider-wrapper <?php echo $extra_class;?>">	
								...
								<div class="product-slider-inner <?php if($box_style !== 'style-boxed') ?> <?php echo esc_attr($show_nav_pos);?> wd-loading">
									
									<?php $current_row = 0;?>
									
									<?php tvlgiao_wpdance_wd_woocommerce_product_loop_start($show_type); ?>
			...
			//<![CDATA[
				jQuery(document).ready(function() {
					...
					res_val[0] = 1;
					res_val[1] = 3;
					res_val[2] = Math.round(temp_visible * 1180 /1200);
					res_val[4] = Math.round(temp_visible * 1180 /1200);
					//res_val[2] = Math.round(temp_visible * 900 /930);
					//res_val[4] = Math.round(temp_visible * 900 /930);
		...
		tvlgiao_wpdance_shop_loop_prod_add_action();
