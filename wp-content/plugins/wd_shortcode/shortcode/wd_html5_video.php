<?php 
	...
	function wd_soundcloud_iframe_widget($options) {
		$url = "https://w.soundcloud.com/player/?url=".$options["url"];
		
		return sprintf('<iframe style = "width:%s; overflow:hidden; border:0;" height="%s" src="%s"></iframe>', $options['width'], $options['height'], esc_url( $url ) );
	}
