...
function tvlgiao_wpdance_wd_get_export_color_theme(){
	global $smof_data;
	$slug = $_POST['slug'];
	$file = $smof_data[$slug];
	...
	$url_xml_file = TVLGiao_Wpdance_THEME_DIR."/config_xml/".$file.".xml";
	...
	if ($res === TRUE) {
		...
		echo TVLGiao_Wpdance_XML_DIR . 'exports/' . $file.'_export.zip';
		die();
	} else {
		unlink($xml_exports.$file."_export.xml");
		echo 'error';
...
add_action('wp_ajax_export_color_theme','tvlgiao_wpdance_wd_get_export_color_theme',10);

function tvlgiao_wpdance_wd_remove_color_theme(){
	$file = $_POST['file'];
	$pa_id = $_POST['pa_id'];
	$color_theme_saved = of_get_options($pa_id);
	$res = array();
	if($file !== $color_theme_saved ) {
		$xml_file = TVLGiao_Wpdance_XML_PATH . $file . '.xml';
		$img_file = TVLGiao_Wpdance_ADMIN_PATH . "assets/images/" . $file . '.png';
		if (file_exists($xml_file) && file_exists($img_file)) {
			...
			$res['status'] = 'error';
			$res['msg'] = esc_html__('Error: Color theme files are not exist.', 'wpnoone');
		}
		
	} else {
		$res['status'] = 'error';
		$res['msg'] = esc_html__('Error: Color theme are using, you can\'t remove it.', 'wpnoone');
	}
	echo json_encode($res);
	die();
}
add_action('wp_ajax_remove_color_theme','tvlgiao_wpdance_wd_remove_color_theme',10);
...
function tvlgiao_wpdance_wd_import_color_theme(){
	global $xml_headers;
	$uploadedfile = $_FILES['file_upload'];
	$xml_dir = get_template_directory() . "/config_xml/";
	$target_dir = TVLGiao_Wpdance_XML_PATH . "tmp/";
	if(file_exists($target_dir)) tvlgiao_wpdance_wd_removeDir($target_dir);
		...
		if (move_uploaded_file($_FILES["file_upload"]["tmp_name"], $target_file)) {
			...
			if($res) {
				...
				if(isset($header_datas['Slug']) && $header_datas['Slug'] !== '') {
					if(!file_exists(TVLGiao_Wpdance_XML_PATH . $header_datas['Slug'].'.xml')) {
						if(copy($tmp_file_png[0],TVLGiao_Wpdance_ADMIN_PATH .'assets/images/'.$header_datas['Slug'].'.png') 
							&& copy($tmp_file_xml[0],TVLGiao_Wpdance_XML_PATH.$header_datas['Slug'].'.xml')) {
							$resturn['status'] = "success";
							$resturn['msg'] = esc_html__('Import success!', 'wpnoone');
						} else {
							$resturn['status'] = 'error';
							$resturn['msg'] = esc_html__('Error: Import color theme not success.', 'wpnoone');
						}
					} else {
						$resturn['status'] = 'error';
						$resturn['msg'] = esc_html__('Error: Color theme slug exist.', 'wpnoone');
					}
				} else {
					$resturn['status'] = 'error';
					$resturn['msg'] = esc_html__('Error: XML syntax error.', 'wpnoone');
				}
					
			} else {
				$resturn['status'] = 'error';
				$resturn['msg'] = esc_html__('Error: Extracting file is error.', 'wpnoone');
			}
		} else {
			$resturn['status'] = 'error';
			$resturn['msg'] = esc_html__('Error: Upload file is error', 'wpnoone');
		}
	}
    tvlgiao_wpdance_wd_removeDir($target_dir);
	echo json_encode($resturn);
	die();
}
add_action('wp_ajax_import_color_theme','tvlgiao_wpdance_wd_import_color_theme',10);

function tvlgiao_wpdance_wd_removeDir($path) {
	if(file_exists($path)) {
		$path = rtrim($path, '/') . '/';
		$items = glob($path . '*');
		foreach($items as $item) {
			is_dir($item) ? tvlgiao_wpdance_wd_removeDir($item) : unlink($item);
		}
		rmdir($path);
	}
}
...
function tvlgiao_wpdance_wd_get_tab_html_content(){
	...
	$url =  TVLGiao_Wpdance_ADMIN_DIR . 'assets/images/';
	$color_image_options = array();
	foreach($xml_arr_file as $xml){
		$header_datas = get_file_data(TVLGiao_Wpdance_XML_PATH . $xml . '.xml', $xml_headers);
	...
	$url_xml_file = TVLGiao_Wpdance_THEME_URI."/config_xml/".$xml_file.".xml";
...
add_action('wp_ajax_tab_refesh','tvlgiao_wpdance_wd_get_tab_html_content',10);


function tvlgiao_wpdance_wd_import_xml(){
	//$file = $_POST['upload_file'];
	//echo json_encode($file);
	echo "kinhdon";
}

add_action('wp_ajax_import_xml','tvlgiao_wpdance_wd_import_xml',10);
