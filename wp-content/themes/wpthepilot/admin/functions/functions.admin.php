...
function of_save_options($data, $key = null) {
	...
	if ($key != null) { // Update one specific value
		if ($key == TVLGiao_Wpdance_BACKUPS) {
...
function tvlgiao_wpdance_wd_of_option_setup()	
{
	global $of_options, $options_machine;
	$tvlgiao_wpdance_wd_data = of_get_options();
	$options_machine = new Options_Machine($of_options);
	$tvlgiao_wpdance_wd_data = tvlgiao_wd_array_atts($options_machine->Defaults, $tvlgiao_wpdance_wd_data);
	tvlgiao_wpdance_wd_of_save_options($tvlgiao_wpdance_wd_data);	
}

function tvlgiao_wpdance_wd_of_save_options($data){
	foreach ( $data as $k=>$v ) {
		if (is_array($v)) {
			foreach ($v as $key=>$val) {
				if ($key != $k && $v[$key] == $val) {
