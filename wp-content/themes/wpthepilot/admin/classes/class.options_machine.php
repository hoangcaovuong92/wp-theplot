...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		$tvlgiao_wpdance_wd_data = $smof_data;
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($smof_data) || !empty($tvlgiao_wpdance_wd_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'sorter':
				    ...
				    $sortlists = isset($tvlgiao_wpdance_wd_data[$value['id']]) && !empty($tvlgiao_wpdance_wd_data[$value['id']]) ? $tvlgiao_wpdance_wd_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$backup = of_get_options(TVLGiao_Wpdance_BACKUPS);
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ','wpnoone').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_import_function($id){
		...
		ob_start();
		?>
			<button style="position:relative; display: inline-table; margin: 0px auto;" data-pa-id="<?php echo esc_attr($id);?>" data-id="<?php echo esc_attr($id);?>_import" type="button" class="button wp_import_button" name="<?php echo esc_attr($id);?>_import_btn"><i class="fa fa-upload"></i> <?php esc_html_e('Import', "wpnoone")?></button>
			<input data-pa-id="<?php echo esc_attr($id);?>" style="position:relative; float: left; width: 65%; display:none;" data-action="<?php echo TVLGiao_Wpdance_ADMIN_DIR?>classes/upload_xml.php" class="wp_import_input" name="<?php echo esc_attr($id);?>_import" id="<?php echo esc_attr($id);?>_import" type="file" />
			<button style="position:relative; display: inline-table;" data-pa-id="<?php echo esc_attr($id);?>" data-action="<?php echo TVLGiao_Wpdance_ADMIN_DIR?>classes/export_xml.php" type="button" class="button-primary wp_export_input" name="<?php echo esc_attr($id);?>_export" title="<?php esc_html_e('Exporting the colour being applied', "wpnoone")?>" id="<?php echo esc_attr($id);?>_export"><i class="fa fa-download"></i> <?php esc_html_e('Export', "wpnoone")?></button>
			<button style="position:relative; display: inline-table;" type="button" class="button-remove wp_remove_button" data-pa-id="<?php echo esc_attr($id);?>" name="<?php echo esc_attr($id);?>_remove" id="<?php echo esc_attr($id);?>_remove"><i class="fa fa-trash-o"></i> <?php esc_html_e('Remove', "wpnoone")?></button>
		<?php
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
	    ...
	    $tvlgiao_wpdance_wd_data = of_get_options();
	    $smof_data = of_get_options();
