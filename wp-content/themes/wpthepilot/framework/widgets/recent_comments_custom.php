...
class WP_Widget_Recent_Comments_custom extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_comments_custom', 'description' => esc_html__( 'The most recent comments','wpnoone' ) );
		parent::__construct('recent-comments-custom', esc_html__('WD - Recent Comments','wpnoone'), $widget_ops);
...
<?php
	...
	function widget( $args, $instance ) {
		...
		if ( isset( $cache[ $args['widget_id'] ] ) ) {
 		...
 		$output = '';
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Recent Comments','wpnoone' ) : $instance['title'], $instance, $this->id_base );
			...
			?>
				<div class="recentcomments_custom <?php if($count==1) echo " first"; if($count==count($comments)) echo " last";?>">
					<div class="detail123">
						...
						<p class="comment-body">"<?php echo  tvlgiao_wpdance_string_limit_words(get_comment_text(),15); ?>"</p>
...
?>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number of comments to show','wpnoone'); ?></label>
