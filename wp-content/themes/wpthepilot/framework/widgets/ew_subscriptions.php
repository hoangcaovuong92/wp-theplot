	...
	class WP_Widget_Ew_subscriptions extends WP_Widget {
		...
		function __construct() {
	    	$widget_setting = array(
				'name' 		=> esc_html__('WD - Feedburner Subscriptions','wpdance'),
				'desc' 		=> esc_html__('Display Subscriptions Form.','wpdance'),
		...
		function widget( $args, $instance ) {
			...
			$feedburner_id = (strlen($feedburner_id) <= 0 ? 'wpnoone' : $feedburner_id);
			...
			<div class="subscribe_widget <?php echo esc_attr($style) . ' '. esc_attr($align);?>">								
				<form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr($feedburner_id);?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
					<p class="subscribe-email"><input type="text" name="email" class="subscribe_email" placeholder="<?php esc_html_e('enter your email address','wpnoone');?>" /></p>
		...
		?>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('intro_text')); ?>"><?php esc_html_e('Enter your Intro Text','wpnoone'); ?> : </label>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('button_text')); ?>"><?php esc_html_e('Enter your Button','wpnoone'); ?> : </label>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style','wpnoone'); ?> : </label>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('align')); ?>"><?php esc_html_e('Text align','wpnoone'); ?> : </label>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('feedburner_id')); ?>"><?php esc_html_e('Enter your Feedburner ID','wpnoone'); ?> : </label>
