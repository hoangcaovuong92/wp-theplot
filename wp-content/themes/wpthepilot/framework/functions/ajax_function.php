<?php 
function tvlgiao_wpdance_wd_aj_data(){
	?>
...
add_action('wp_head', 'tvlgiao_wpdance_wd_aj_data');

add_action('wp_ajax_nopriv_more_post_ajax', 'tvlgiao_wpdance_more_post_ajax_blog'); 
add_action('wp_ajax_more_post_ajax', 'tvlgiao_wpdance_more_post_ajax_blog');

function tvlgiao_wpdance_more_post_ajax_blog(){ 
		...
		while ($custom2->have_posts()) : $custom2->the_post(); 
		   $_post_config = get_post_meta($custom2->post->ID,TVLGiao_Wpdance_THEME_SLUG.'custom_post_config',true);
		...
		<div class="item-content">
			...
			<?php if(isset($_post_config['post_type'])):?>
								...
								case 'video':
									if(strlen(trim($_post_config['video_url'])) > 0){
										$video_url = trim($_post_config['video_url']);
											if (!empty($video_url)) {
												$var = apply_filters('the_content', '[embed width="1200" height="240"]' . $video_url . '[/embed]');
												echo  $var;
											}
										}
										break;
					 ...
					 <?php $gallery_ids = get_post_meta($custom2->post->ID, TVLGiao_Wpdance_THEME_SLUG.'post_gallery', true);
								if( $gallery_ids != '' ){
										...
										<?php foreach( $gallery_ids as $id ): ?>
											<div class="image">
												<a class="thumb-image" href="<?php the_permalink() ; ?>">
													<?php echo wp_get_attachment_image( $id, 'tvlgiao_wpdance_blog_thumb' ); ?>
												</a>
											</div>
										<?php endforeach; ?>
									</div>
			...
			<div class="meta-post post-info-content">
					...
					<div class="post-info-meta-top post-info-meta">						
						...
						else echo absint($comments_count->approved) . ' ' . _n( "Comment", "Comments", absint($comments_count->approved), 'wpnoone');?>
						</div>
						<?php endif;?>
					...
					<p class="excerpt"><?php 
						...
						tvlgiao_wpdance_the_excerpt_max_words($words_limit,$custom2->post); ?>
					</p>
					<?php endif;?>
					<?php if( $tvlgiao_wpdance_wd_data['wd_blog_readmore'] == 1 ) : ?>		
					<a class="button" href="<?php esc_url(the_permalink()); ?>"><?php esc_html_e("Read more", "wpnoone");?></a>
					<?php endif;?>
