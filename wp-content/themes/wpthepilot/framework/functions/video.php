<?php 
if(!function_exists ('tvlgiao_wpdance_wd_parse_daily_link')){
	function tvlgiao_wpdance_wd_parse_daily_link($daily_link){
...
if(!function_exists ('tvlgiao_wpdance_wd_get_embbed_vimeo')){
	function tvlgiao_wpdance_wd_get_embbed_vimeo($video_url,$width,$height){
		return '<div class = "blog_video"><iframe src="//player.vimeo.com/video/'.tvlgiao_wpdance_wd_parse_vimeo_link($video_url).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="'.$width.'" height="'.$height.'" style="border:0;" allowFullScreen></iframe></div>';
	}
}


if(!function_exists ('tvlgiao_wpdance_wd_get_embbed_video')){
	function tvlgiao_wpdance_wd_get_embbed_video($video_url,$width = 940,$height = 558){
		if(strstr($video_url,'youtube.com') || strstr($video_url,'youtu.be')){
			return "<iframe id='youtube-iframe' title='YouTube video player' class='youtube-player' type='text/html'
				 width='{$width}' height='{$height}' src='http://www.youtube.com/embed/".tvlgiao_wpdance_wd_parse_youtube_link($video_url)."?wmode=transparent'
