...
<?php 
	...
	if( true ) {
		if ( is_category() ) {
			...
			$page_title .= sprintf( __( 'Category : %s', 'wpnoone' ), single_cat_title( '', false ) );
			$page_title .= "</h1>";
		}
		elseif ( is_search() ) {
			$page_title .= '<h1 class="search-title page-title heading-title site-title">';
			$page_title .= sprintf( __( 'Search Results for : %s', 'wpnoone' ), get_search_query() );
			$page_title .= '</h1>';
			 
		}elseif ( is_day() ) {
			$page_title .= '<h1 class="page-title heading-title archive-title site-title">';
			$page_title .= sprintf( __( 'Day : %s', 'wpnoone' ), get_the_date() );
			$page_title .= '</h1>';
		} elseif ( is_month() ) {
			$page_title .= '<h1 class="page-title heading-title archive-title  site-title">';
			$page_title .= sprintf( __( 'Month : %s', 'wpnoone' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'wpnoone' ) ) ); 
			$page_title .= '</h1>';
	 
		} elseif ( is_year() ) {
			$page_title .= '<h1 class="page-title heading-title archive-title site-title">';
			$page_title .= sprintf( __( 'Year : %s', 'wpnoone' ), get_the_date( _x( 'Y', 'yearly archives date format', 'wpnoone' ) ) ); 
			$page_title .= '</h1>';
			...
			$page_title .= sprintf( __( 'Tag : %s', 'wpnoone' ), single_tag_title( '', false ) );
			$page_title .= '</h1>';
			...
			$page_title .= sprintf( __( 'Author : %s', 'wpnoone' ), $userdata->display_name );
			$page_title .= '</h1>';
	 
		} elseif ( is_404() ) {
			$page_title .= '<h1 class="page-title heading-title title-404 page-title site-title">';
			$page_title .= esc_html__( 'OOPS! FILE NOT FOUND', 'wpnoone' );
			$page_title .= '</h1>';
		} elseif( is_archive() ){
			$page_title .= '<h1 class="page-title heading-title attachment-title single-title site-title">';
			$page_title .= esc_html__( 'Archive', 'wpnoone' );
			$page_title .= '</h1>';
		}
	}
			...
			<div id="content" class="container" role="main">
				...
				<div id="container-main" class="<?php echo esc_attr($_main_class);?>">
					<div class="main-content">
						...
						?>
							<div class="page_navi">
								<!--div class="nav-previous"><?php previous_posts_link( esc_html__( 'Prev Entries', 'wpnoone' ) ); ?></div-->
								<div class="nav-content"><?php tvlgiao_wpdance_ew_pagination();?></div>
								<!--div class="nav-next"><?php next_posts_link( esc_html__( 'Next Entries', 'wpnoone' ) ); ?></div-->
							</div>
						<?php } ?>
					<?php wp_reset_postdata();?>
